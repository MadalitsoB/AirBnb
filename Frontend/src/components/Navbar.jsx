  ...
  return (
    <div className="mini-cal">
      <div className="mini-cal__nav">
        <button type="button" className="mini-cal__nav-btn" onClick={prevMonth}>‹</button>
        <span className="mini-cal__heading">{MONTHS[viewMonth]} {viewYear}</span>
        <button type="button" className="mini-cal__nav-btn" onClick={nextMonth}>›</button>
      </div>
      <div className="mini-cal__grid">
        {DAYS.map(d => <span key={d} className="mini-cal__dow">{d}</span>)}
        {cells.map((day, i) => {
  ...
  return (
    <div className="guests-counter">
      ...
      <div className="guests-counter__controls">
        <button type="button" className="guests-counter__btn" onClick={onDec} disabled={value <= min}>−</button>
        <span className="guests-counter__num">{value}</span>
        <button type="button" className="guests-counter__btn" onClick={onInc} disabled={value >= max}>+</button>
      </div>
    </div>
  );
}
...
function Navbar() {
  ...
  const [availableLocations, setAvailableLocations] = useState(FALLBACK_LOCATIONS);
  ...
  useEffect(() => {
    apiFetch("/accommodations").then((data) => {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (locationRef.current  && !locationRef.current.contains(e.target))  { setLocationOpen(false);  setLocationQuery(""); }
      if (checkInRef.current   && !checkInRef.current.contains(e.target))   setCheckInOpen(false);
      if (checkOutRef.current  && !checkOutRef.current.contains(e.target))  setCheckOutOpen(false);
  ...
  useEffect(() => {
    if (locationOpen && locationInputRef.current) locationInputRef.current.focus();
  }, [locationOpen]);

  const closeAll = () => {
    setLocationOpen(false); setCheckInOpen(false);
    setCheckOutOpen(false); setGuestsOpen(false);
    setLocationQuery("");
  };
  ...
  const filteredLocations = locationQuery.trim()
    ? availableLocations.filter(l => l.toLowerCase().includes(locationQuery.toLowerCase()))
    : availableLocations;
  ...
  return (
    <header className="navbar">
      <div className="navbar__inner">
        ...
        <div className={`navbar__search ${isHost ? "navbar__search--hidden" : ""}`}>
          <div className="navbar__search-pill">
            ...
            <div className="navbar__search-field" ref={locationRef}>
              <button type="button"
                ...
                onClick={() => { setLocationOpen(v => !v); setCheckInOpen(false); setCheckOutOpen(false); setGuestsOpen(false); setMenuOpen(false); }}>
                <span className="navbar__search-label">Location</span>
                <span className={`navbar__search-value ${!location ? "navbar__search-value--placeholder" : ""}`}>
              ...
              {locationOpen && (
                <div className="search-dropdown search-dropdown--location">
                  <div className="search-dropdown__search-wrap">
                    <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="#717171" strokeWidth="1.5">
                    ...
                    {locationQuery && (
                      <button type="button" className="search-dropdown__search-clear" onClick={() => setLocationQuery("")}>×</button>
                    )}
                  </div>
                  <p className="search-dropdown__title">
                    {locationQuery ? `Results for "${locationQuery}"` : "Available destinations"}
                  </p>
                  <div className="search-dropdown__list">
                    {filteredLocations.length > 0 ? filteredLocations.map(loc => (
                      <button key={loc} type="button"
                        ...
                        onClick={() => { setLocation(loc); setLocationOpen(false); setLocationQuery(""); }}>
                        <span className="search-dropdown__item-icon">📍</span>
                        <span className="search-dropdown__item-text">
                          {locationQuery ? (() => {
                            const idx = loc.toLowerCase().indexOf(locationQuery.toLowerCase());
                            if (idx === -1) return loc;
                            return <>{loc.slice(0,idx)}<strong>{loc.slice(idx, idx+locationQuery.length)}</strong>{loc.slice(idx+locationQuery.length)}</>;
                          })() : loc}
                        </span>
                        {location === loc && (
                          <svg viewBox="0 0 16 16" width="14" height="14" style={{marginLeft:"auto",flexShrink:0}}>
                            <path d="M2 8l4 4 8-8" stroke="#FF385C" strokeWidth="2" fill="none" strokeLinecap="round"/>
                          </svg>
                        )}
                      </button>
                    )) : (
                      <p className="search-dropdown__empty">No destinations match "{locationQuery}"</p>
                    )}
                  </div>
                  {location && (
                    <button type="button" className="search-dropdown__clear"
                      onClick={e => { e.stopPropagation(); setLocation(""); setLocationQuery(""); }}>
                      Clear selection
                    </button>
                  )}
                </div>
            ...
            <div className="navbar__search-field" ref={checkInRef}>
              <button type="button"
                ...
                onClick={() => { setCheckInOpen(v => !v); setLocationOpen(false); setCheckOutOpen(false); setGuestsOpen(false); setMenuOpen(false); }}>
                <span className="navbar__search-label">Check in</span>
                <span className={`navbar__search-value ${!checkIn ? "navbar__search-value--placeholder" : ""}`}>
              ...
              {checkInOpen && (
                <div className="search-dropdown search-dropdown--calendar">
                  <MiniCalendar
                    value={checkIn}
                    minDate={today}
                    onChange={d => { setCheckIn(d); if (checkOut && d > checkOut) setCheckOut(""); }}
                    onClose={() => { setCheckInOpen(false); setCheckOutOpen(true); }}
                  />
                </div>
              )}
            </div>
            ...
            <div className="navbar__search-field" ref={checkOutRef}>
              <button type="button"
                ...
                onClick={() => { setCheckOutOpen(v => !v); setLocationOpen(false); setCheckInOpen(false); setGuestsOpen(false); setMenuOpen(false); }}>
                <span className="navbar__search-label">Check out</span>
                <span className={`navbar__search-value ${!checkOut ? "navbar__search-value--placeholder" : ""}`}>
            ...
            <div className="navbar__search-field" ref={guestsRef}>
              <button type="button"
                ...
                onClick={() => { setGuestsOpen(v => !v); setLocationOpen(false); setCheckInOpen(false); setCheckOutOpen(false); setMenuOpen(false); }}>
                <span className="navbar__search-label">Guests</span>
                <span className={`navbar__search-value ${totalGuests === 0 ? "navbar__search-value--placeholder" : ""}`}>
                  {guestsLabel}
                </span>
              </button>
              ...
              {guestsOpen && (
                <div className="search-dropdown search-dropdown--guests">
                  ...
                  {totalGuests > 0 && (
                    <button type="button" className="search-dropdown__clear"
                      onClick={() => { setAdults(0); setChildren(0); }}>
                      Clear guests
                    </button>
                  )}
                </div>
            ...
            {/* SEARCH BTN */}
            <button type="button" className="navbar__search-btn" onClick={handleSearch} aria-label="Search stays">
        ...
        <div className="navbar__right">
          ...
          <div className="navbar__user" onClick={() => { setMenuOpen(v => !v); closeAll(); }}>
            <svg viewBox="0 0 32 32" width="16" height="16" fill="currentColor">
              <path d="M3 8h26M3 16h26M3 24h26" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
            <div className="navbar__avatar">
              {user ? (
                <span className="navbar__avatar-initial">{user.username?.[0]?.toUpperCase() || "U"}</span>
              ) : (
