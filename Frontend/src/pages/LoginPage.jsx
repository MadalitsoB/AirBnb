...
function LoginPage() {
  ...
  /* Step 1 — probe whether the email exists */
  const handleContinue = async (e) => {
    ...
    try {
      ...
      setIsNew(!(raw.includes("invalid") || raw.includes("password") || raw.includes("incorrect")));
    }
    setLoading(false);
    setStep(2);
  };
    ...
    return (
      <div className="auth-overlay">
        <div className="host-auth-modal">
          ...
          {/* Close */}
          <button className="host-auth-modal__close" onClick={() => navigate("/")}>
          ...
          {/* Left — branding panel */}
          <div className="host-auth-modal__left">
            <div className="host-auth-modal__brand">
            ...
            <div className="host-auth-modal__left-body">
              <p className="host-auth-modal__eyebrow">For hosts</p>
              <h2 className="host-auth-modal__headline">
                {step === 1 ? "Start hosting\ntoday." : isNew ? "Create your\nhost account." : "Welcome\nback."}
              </h2>
              <p className="host-auth-modal__sub">
                {step === 1
                  ? "List your space, set your price, and host guests from across South Africa and beyond."
                  : isNew
                  ? "You're one step away from publishing your first listing."
                  : "Your dashboard and listings are waiting for you."}
              </p>
            </div>
          </div>

          {/* Right — form panel */}
          <div className="host-auth-modal__right">
            <h3 className="host-auth-modal__form-title">
              {step === 1 ? "Log in or sign up" : isNew ? "Create account" : "Log in"}
            </h3>

            {step === 1 ? (
              <>
                <form onSubmit={handleContinue} className="host-auth-form">
                  <input
                    type="email"
                    ...
                    autoFocus
                  />
                  {message && <p className="host-auth-error">{message}</p>}
                  <button type="submit" className="host-auth-btn" disabled={loading}>
                    {loading ? "Checking…" : "Continue →"}
                  </button>
                </form>

                <div className="host-auth-divider"><span>or continue with</span></div>

                <div className="host-auth-social">
                  <button type="button" className="host-auth-social-btn" aria-label="Google">
                  ...
                  <button type="button" className="host-auth-social-btn" aria-label="Apple">
              ...
              <>
                <p className="host-auth-email-recap">
                  {email}
                  <button type="button" onClick={() => { setStep(1); setMessage(""); setPassword(""); }}>
                    Change
                  </button>
                </p>
                <form onSubmit={handleSubmit} className="host-auth-form">
                  {isNew && (
                    <input
                      type="text"
                  ...
                  <button type="submit" className="host-auth-btn" disabled={loading}>
                    {loading
                      ? isNew ? "Creating…" : "Logging in…"
                      : isNew ? "Create host account →" : "Log in →"}
                  </button>
                </form>
              </>
            )}
  ...
  /* ── Guest flow modal (unchanged Airbnb style) ── */
  return (
    <div className="auth-overlay">
